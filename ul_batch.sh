...
for NEW_VALUE in "${NEW_VALUES[@]}"; do
  ...
  sed -i "s|./build_CascadeLake/ContinuumSystem params/ul_cluster.config|./build_CascadeLake/ContinuumSystem auto_generated_ul_${CURRENT_TIME}/$NEW_NAME.config|" auto_generated_ul_${CURRENT_TIME}/$NEW_NAME.slurm
