...
SCModel::SCModel(ModelInitializer const& parameters)
    : Delta(2 * DISCRETIZATION + 1, c_complex{}),
      ...
      phonon_coupling{parameters.phonon_coupling},
      omega_debye{parameters.omega_debye},
      fermi_energy{parameters.fermi_energy},
      screening_ratio{parameters.screening_ratio},
    ...
    std::cout << "Fock(k_F) = " << __fock_coulomb(fermi_wavevector)
              << "  xi(k_F) = " << dispersion_to_fermi_level(fermi_wavevector) << std::endl;
    Delta = decltype(Delta)::FromAllocator(
        [&](int i) -> c_complex {
            const c_float k = momentumRanges.index_to_momentum(i);
            const c_float magnitude =
                (k < std::sqrt(2. * (fermi_energy - omega_debye)) || k > std::sqrt(2. * (fermi_energy + omega_debye)))
                    ? 0.01
                    : 0.1;
            if (i < DISCRETIZATION) {
#ifdef _complex
...
void SCModel::set_new_parameters(ModelInitializer const& parameters) {
    ...
    if (is_zero(Delta[DISCRETIZATION / 2])) {
        Delta = decltype(Delta)::FromAllocator(
            [&](int i) -> c_complex {
                const c_float k = momentumRanges.index_to_momentum(i);
                const c_float magnitude = (k < std::sqrt(2. * (fermi_energy - omega_debye)) ||
                                           k > std::sqrt(2. * (fermi_energy + omega_debye)))
                                              ? 0.01
                                              : 0.1;
                if (i < DISCRETIZATION) {
#ifdef _complex
                    return std::polar(magnitude,
                                      1.3 + PI * (static_cast<double>(i) / static_cast<double>(DISCRETIZATION)));
#else
                    return magnitude *
                           std::cos(PI * (static_cast<double>(i) / static_cast<double>(DISCRETIZATION) - 0.5));
#endif
                }
                if (i == 2 * DISCRETIZATION)
                    return c_complex{};
...
c_complex SCModel::k_infinity_integral() const {
    auto integrand = [this](c_float q) { return q * q * this->sc_expectation_value(q); };
...
c_float SCModel::fock_coulomb(c_float k) const {
    ...
    const c_float ln_factor{(screening * screening + fermi_wavevector * fermi_wavevector - k * k) /
                            (2.0 * k * fermi_wavevector)};
    return -coulomb_scaling * PhysicalConstants::em_factor * fermi_wavevector *
...
c_float SCModel::occupation_index(int k) const {
    ...
    const c_float E = std::sqrt(eps_mu * eps_mu + std::norm(Delta[k]));
    if (is_zero(temperature)) {
...
void SCModel::iteration_step(const ParameterVector& initial_values, ParameterVector& result) {
    // static int step_num = 0; // For Debug purposes
    // ++step_num;
    result.setZero();
    ...
    auto delta_n_wrapper = [this](c_float q) { return this->delta_n(q); };
#endif
...
c_float SCModel::compute_coefficient(mrock::symbolic_operators::Coefficient const& coeff,
                                     c_float first,
                                     c_float second) const {
    if (coeff.name == "\\epsilon_0") {
        ...
        return coulomb_scaling / (2 * first * second * PhysicalConstants::vacuum_permitivity) *
               log_expression(first + second, first - second);
...
const std::map<mrock::symbolic_operators::OperatorType, std::vector<c_complex>>& SCModel::get_expectation_values()
    const {
    if (_expecs.empty()) {
...
c_complex SCModel::interpolate_delta(c_float k) const {
    ...
    if (index < 0)  // Assuming Delta(k) = 0 for k->0
        return c_complex{};
    return mrock::utility::Numerics::interpolate_from_vector<n_interpolate>(k, momentumRanges, Delta,
                                                                            shifted_index(index));
}
...
c_float SCModel::interpolate_fock_correction(c_float k) const {
    ...
    if (index < 0)  // Assuming Delta(k) = const for k->0
        return c_float{};
    return std::real(mrock::utility::Numerics::interpolate_from_vector<n_interpolate>(
        k, momentumRanges, Delta, shifted_index(index), DISCRETIZATION));
}
...
std::string SCModel::info() const {
    return "SCModel // T=" + std::to_string(temperature) + "K   g=" + std::to_string(phonon_coupling) +
           "   omega_D=" + std::to_string(1e3 * omega_debye) + "meV   E_F=" + std::to_string(fermi_energy) +
           "eV   alpha=" + std::to_string(coulomb_scaling) + "   lambda=" + std::to_string(screening) + "sqrt(eV)";
}
...
std::string SCModel::to_folder() const {
    ...
    return "T=" + improved_string(temperature) + "/coulomb_scaling=" + improved_string(coulomb_scaling) +
           "/screening=" + improved_string(screening_ratio) + "/k_F=" + improved_string(fermi_wavevector) +
           "/g=" + improved_string(phonon_coupling) + "/omega_D=" + improved_string(1e3 * omega_debye) + "/";
}
...
c_complex SCModel::delta_max() const {
    return std::abs(
        *std::max_element(Delta.begin(), Delta.begin() + DISCRETIZATION,
                          [](const c_complex& lhs, const c_complex& rhs) { return std::abs(lhs) < std::abs(rhs); }));
}
...
void SCModel::set_splines() {
    ...
    this->sc_expectation_value = SplineContainer(
        _expecs[mrock::symbolic_operators::OperatorType::SC], momentumRanges.K_MIN, momentumRanges.LOWER_STEP,
        momentumRanges.INNER_STEP, momentumRanges.UPPER_STEP, _OUTER_DISC, _INNER_DISC);
}
}  // namespace Continuum