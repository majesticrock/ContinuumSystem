...
MomentumRanges::MomentumRanges(c_float const* k_F, const c_float omega_debye, const c_float inner_offset)
    : K_MAX{2 * (*k_F)},
...
#else
      INNER_K_MAX{std::sqrt((*k_F * *k_F) + 2 * APPROX_INNER_OFFSET * omega_debye)},
      INNER_K_MIN{std::sqrt((*k_F * *k_F) - 2 * APPROX_INNER_OFFSET * omega_debye)},
#endif
      ...
      K_F{k_F},
      singularities_in_renormalized_dispersion{std::sqrt(*k_F * *k_F - 2. * omega_debye),
                                               std::sqrt(*k_F * *k_F + 2. * omega_debye)} {
    ...
    for (int i = 0; i < DISCRETIZATION; ++i) {
        ...
        if (i != momentum_to_index(k)) {
            std::cerr << "MomentumRanges.cpp:" << __LINE__ << " || " << i << " -> " << k << " -> "
                      << momentum_to_index(k) << std::endl;
            throw -1;
        }
    }
#endif
