...
c_float ModelInitializer::compute_fermi_energy() const {
    ...
    const c_float fock =
        -PhysicalConstants::em_factor * coulomb_scaling * fermi_wavevector *
...
std::ostream& operator<<(std::ostream& os, ModelInitializer const& init) {
    os << "T=" << init.temperature << " K   " << "g=" << init.phonon_coupling << "   " << "omega_D=" << init.omega_debye
       << " eV   " << "E_F=" << init.fermi_energy << " eV   " << "alpha=" << init.coulomb_scaling << "   "
       << "k_F=" << init.fermi_wavevector << " sqrt(eV)   " << "rho_F=" << init.rho_F << " sqrt(eV)   ";
    return os;
}
}  // namespace Continuum