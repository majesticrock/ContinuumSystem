...
namespace Continuum {
c_float ModeHelper::compute_momentum(mrock::symbolic_operators::Momentum const& momentum,
                                     c_float k,
                                     c_float l,
                                     c_float q /*=0*/) const {
...
void ModeHelper::create_starting_states() {
    ...
    std::fill(starting_states[0].begin(), starting_states[0].begin() + m_iterator::max_idx(),
              std::sqrt(model->momentumRanges.INNER_STEP));
    std::fill(starting_states[1].begin() + 3 * m_iterator::max_idx(), starting_states[1].end(),
              std::sqrt(model->momentumRanges.INNER_STEP));
#else
    starting_states.push_back({iEoM_algorithm::Vector::Zero(antihermitian_discretization),
                               iEoM_algorithm::Vector::Zero(hermitian_discretization), "SC"});
    for (m_iterator it(&model->momentumRanges); it < m_iterator::max_idx(); ++it) {
        starting_states[0][0](it.idx) = 1. / std::sqrt((c_float)DISCRETIZATION);
        starting_states[0][1](it.idx) = 1. / std::sqrt((c_float)DISCRETIZATION);
    }
#endif
}
...
void ModeHelper::fill_matrices() {
    ...
    /* for (m_iterator it(&model->momentumRanges); it < m_iterator::max_idx(); ++it) {
            std::cout << it.k / model->fermi_wavevector << " -> " << K_plus(it.idx, m_iterator::max_idx() + it.idx) << "
    " << K_plus(m_iterator::max_idx() + it.idx, it.idx) << " || "
                    << - 2 * model->interpolate_delta(it.k) * model->dispersion_to_fermi_level(it.k) /
    model->energy(it.k)  << std::endl;
    } */
#ifdef _XP
    ...
    for (m_iterator it(&model->momentumRanges); it < m_iterator::max_idx(); ++it) {
        ...
        for (const auto& term : wicks.M[number_of_basis_terms * j + i]) {
            if (!term.delta_momenta.empty()) {
                ...
                // M(i * m_iterator::max_idx() + it.idx, j * m_iterator::max_idx() + it.idx) += iEoM_diag(it.k) *
                // computeTerm(term, it.k, it.k);
            } else {
                for (m_iterator jt(&model->momentumRanges); jt < m_iterator::max_idx(); ++jt) {
                    if (is_zero(jt.k))
                        continue;
                    M(i * m_iterator::max_idx() + it.idx, j * m_iterator::max_idx() + jt.idx) +=
    ...
    for (m_iterator it(&model->momentumRanges); it < m_iterator::max_idx(); ++it) {
        ...
        for (const auto& term : wicks.N[number_of_basis_terms * j + i]) {
            if (!term.delta_momenta.empty()) {
                ...
                N(i * m_iterator::max_idx() + it.idx, j * m_iterator::max_idx() + it.idx) +=
                    computeTerm(term, it.k, it.k);
            } else {
                throw std::runtime_error("Offdiagonal term in N!");
                /* for (m_iterator jt(&model->momentumRanges); jt < m_iterator::max_idx(); ++jt) {
                        N(i * m_iterator::max_idx() + it.idx, j * m_iterator::max_idx() + jt.idx) += iEoM_offdiag(it.k,
                jt.k) * computeTerm(term, it.k, jt.k); if(is_zero(jt.k)) continue;
                } */
            }
        }
    ...
    if (term.sums.momenta.empty()) {
        ...
        if (!term.coefficients.empty()) {
            ...
            if (coeff_ptr->momenta.size() == 2U) {
                value *= model->compute_coefficient(*coeff_ptr, compute_momentum(coeff_ptr->momenta[0], k, l),
                                                    compute_momentum(coeff_ptr->momenta[1], k, l));
...
std::vector<c_float> ModeHelper::continuum_boundaries() const {
    ...
    c_float prev_min{std::numeric_limits<double>::max()};
    c_float min{std::numeric_limits<double>::max()};
    ...
    std::cout << "Found min at " << min / model->fermi_wavevector << "k_F   E(min)=" << model->energy(min)
              << "   E(k_F)=" << model->energy(model->fermi_wavevector) << std::endl;
    return {2 * model->energy(min), 2 * std::max(model->energy(buf.max_k()), model->energy(buf.min_k()))};
}
...
ModeHelper::ModeHelper(ModelInitializer const& init)
...
#ifdef _XP
                     m_iterator::max_idx() * hermitian_size,
                     m_iterator::max_idx() * antihermitian_size,
                     false,
#endif
                     false) {
    ...
    const m_iterator buf(&model->momentumRanges);
    std::cout << "LB: <f_k> = " << model->sc_expectation_value(buf.min_k())
              << "  k_min = " << buf.min_k() / model->fermi_wavevector << "  E_min = " << model->energy(buf.min_k())
              << "\nUB: <f_k> = " << model->sc_expectation_value(buf.max_k())
              << "  k_max = " << buf.max_k() / model->fermi_wavevector << "  E_max = " << model->energy(buf.max_k())
              << std::endl;
}
}  // namespace Continuum