...
void PhononInteraction::compute_renormalization_table() {
    ...
    for (MomentumIterator it(&parent->momentumRanges); it < MomentumIterator::max_idx(); ++it) {
        ...
        const c_float sqrt_minus = std::sqrt(std::abs(it.k * it.k - *ptr_omega_debye));
        const c_float sqrt_plus = std::sqrt(it.k * it.k + *ptr_omega_debye);

        const c_float term_alpha =
            (it.k * it.k < *ptr_omega_debye
                 ? sqrt_minus * (std::atan(*ptr_fermi_wavevector / sqrt_minus) - M_PI_2)
                 : sqrt_minus * std::log((std::abs(*ptr_fermi_wavevector - sqrt_minus) + CUT_REGULARIZATION) /
                                         (std::abs(*ptr_fermi_wavevector + sqrt_minus) + CUT_REGULARIZATION)));
        const c_float term_beta =
            sqrt_plus * std::log((std::abs(*ptr_fermi_wavevector - sqrt_plus) + CUT_REGULARIZATION) /
                                 (std::abs(*ptr_fermi_wavevector + sqrt_plus) + CUT_REGULARIZATION));
    ...
    auto compute_at_k = [this](c_float k) {
        ...
        return mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<
                   c_float, 120>::generalized_principal_value(integrand_fock_alpha, parent->fermi_wavevector,
                                                              parent->momentumRanges.K_MAX /* infinity */,
                                                              singularities[0]) +
               boost::math::quadrature::gauss<c_float, 10000>::integrate(
                   integrand_fock_alpha, parent->momentumRanges.K_MAX, 1e4 /* infinity */) +
               mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<
                   c_float, 120>::generalized_principal_value(integrand_fock_beta, parent->momentumRanges.K_MIN,
                                                              parent->fermi_wavevector, singularities[1]);
    };
    ...
    for (MomentumIterator it(&parent->momentumRanges); it < MomentumIterator::max_idx(); ++it) {
        try {
            singularities_cache[it.idx][0] = mrock::utility::Numerics::Roots::bisection(
                [this, &it](c_float q) { return alpha_CUT(q, it.k); }, parent->momentumRanges.K_MIN,
                parent->momentumRanges.K_MAX, PRECISION, 200);
        ...
        try {
            singularities_cache[it.idx][1] = mrock::utility::Numerics::Roots::bisection(
                [this, &it](c_float q) { return beta_CUT(q, it.k); }, parent->momentumRanges.K_MIN,
                parent->momentumRanges.K_MAX, PRECISION, 200);
        } catch (mrock::utility::Numerics::Roots::NoRootException const& e) {
...
std::array<c_float, 2> PhononInteraction::get_singularities(c_float k) const {
    ...
    return {mrock::utility::Numerics::linearly_interpolate(
                k, parent->momentumRanges[index], parent->momentumRanges[index + 1], singularities_cache[index][0],
                singularities_cache[index + 1][0]),
            mrock::utility::Numerics::linearly_interpolate(
                k, parent->momentumRanges[index], parent->momentumRanges[index + 1], singularities_cache[index][1],
                singularities_cache[index + 1][1])};
}
...
#endif
    auto func = [&](c_float l) { return parent->phonon_boundary_b(l, ALPHA); };
#ifdef approximate_theta
    return mrock::utility::Numerics::Roots::bisection(func, parent->momentumRanges.K_MIN, parent->fermi_wavevector,
                                                      PRECISION, 200);
#else
    const auto lb = func(parent->momentumRanges.K_MIN);
    const auto ub = func(k);
    if (lb * ub >= c_float{})
        return parent->momentumRanges.K_MIN;
...
c_float PhononInteraction::sc_channel_upper_bound(c_float k) const {
    ...
    auto func = [&](c_float l) { return parent->phonon_boundary_b(l, ALPHA); };
#ifdef approximate_theta
    return mrock::utility::Numerics::Roots::bisection(func, parent->fermi_wavevector, parent->momentumRanges.K_MAX,
                                                      PRECISION, 200);
#else
    const auto lb = func(k);
    const auto ub = func(parent->momentumRanges.K_MAX);
    if (lb * ub >= c_float{})
        return parent->momentumRanges.K_MAX;
...
c_float PhononInteraction::renormalization_flow(c_float k) const {
    ...
    return mrock::utility::Numerics::linearly_interpolate(
        k, parent->momentumRanges[index], parent->momentumRanges[index + 1], renormalization_cache[index][0],
        renormalization_cache[index + 1][0]);
}
...
c_float PhononInteraction::fock_correction(c_float k) const {
    ...
    return mrock::utility::Numerics::linearly_interpolate(
        k, parent->momentumRanges[index], parent->momentumRanges[index + 1], renormalization_cache[index][1],
        renormalization_cache[index + 1][1]);
}
...
c_float PhononInteraction::fock_channel(c_float k, c_float k_prime) const {
    ...
    return factor * (__inverse(beta_CUT(k, k_prime)) - __inverse(alpha_CUT(k, k_prime)));
}
}  // namespace Continuum