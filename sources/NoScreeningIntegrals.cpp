...
inline auto I_1(ExpectationValues const& expecs, c_float k) const {
    auto integrand = [&expecs, &k](c_float x) {
        ...
        return expecs(k * tanh_x) * x * tanh_x / mrock::utility::constexpr_power<2>(std::cosh(0.5 * x));
    };

    const c_float alpha{momentumRanges.K_MIN / k};  //
    if (is_zero(1. - alpha))
        return decltype(expecs(k)){};
    ...
    if (k > fermi_wavevector) {
        ...
        if (k > fermi_wavevector + k_f_offset) {
            ...
            return prefactor *
                   (CUT_OFF_CONSTANT * expecs(k) +
                    boost::math::quadrature::gauss<double, n_gauss>::integrate(integrand, lower_bound, middle_bound) +
                    boost::math::quadrature::gauss<double, n_gauss>::integrate(integrand, middle_bound,
                                                                               middle_middle_bound) +
                    boost::math::quadrature::gauss<double, n_gauss>::integrate(integrand, middle_middle_bound,
                                                                               upper_bound));
        }
    ...
    return prefactor * (CUT_OFF_CONSTANT * expecs(k) + boost::math::quadrature::gauss<double, n_gauss>::integrate(
                                                           integrand, lower_bound, upper_bound));
}
...
inline auto I_2(ExpectationValues const& expecs, c_float k) const {
    auto integrand = [&expecs, &k](c_float x) {
        ...
        return expecs(k * coth_x) * x * coth_x / mrock::utility::constexpr_power<2>(std::sinh(0.5 * x));
    };
