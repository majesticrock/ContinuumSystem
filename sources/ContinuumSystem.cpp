...
#pragma omp parallel for
    for (int U = 0; U < N_points; ++U) {
        ...
        SCModel model(init);
        mrock::utility::Selfconsistency::IterativeSolver<c_complex, SCModel, ModelAttributes<c_complex>> solver(
            &model, &model.Delta);
        solver.compute();
...
#define RANK_RANGES(x)                                       \
    const double rank_range = (std::stod(argv[3]) - init.x); \
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n_iter; ++i) {
        ...
        const std::string output_folder = input.getString("output_folder") + "/" +
                                          "N_k=" + std::to_string(DISCRETIZATION) + "/" + modes.getModel().to_folder();
        ...
        auto generate_comments = [&]() {
            return nlohmann::json{
                ...
                {"k_infinity_factor", std::real(2. * PhysicalConstants::em_factor * modes.getModel().coulomb_scaling *
                                                delta_result[2 * DISCRETIZATION])},
            ...
            nlohmann::json{{"data",
                            {
                                ...
                                {"Delta_Fock", as_meV(std::vector<double>(delta_result.begin() + DISCRETIZATION,
                                                                          delta_result.begin() + 2 * DISCRETIZATION))},
...
#else   // CONTINUUM_FULL_DIAG
        {
            ...
            nlohmann::json jFullDiag = {{"phase", full_diag_data.first},
                                        {"amplitude", full_diag_data.second},
                                        {"continuum_boundaries", modes.continuum_boundaries()}};
            jFullDiag.merge_patch(generate_comments());
            mrock::utility::save_string(jFullDiag.dump(4),
                                        std::string(OUTPUT_DATA_DIR) + output_folder + "full_diagonalization.json.gz");
        }
